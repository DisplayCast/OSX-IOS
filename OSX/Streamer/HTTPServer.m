...
- (CGImageRef)allocMaskedScreenImage:(CGImageRef)image {
	CGColorSpaceRef colorspace = CGImageGetColorSpace(image);
	CGContextRef context = CGBitmapContextCreate(NULL, CGImageGetWidth(image), CGImageGetHeight(image),
...
- (CGImageRef)newResizedCGImage:(CGImageRef)image toWidth:(unsigned long)width andHeight:(unsigned long)height {
	CGColorSpaceRef colorspace = CGImageGetColorSpace(image);
...
- (void)performDefaultRequestHandling:(HTTPServerRequest *)mess {
    ...
    if ([method isEqual:@"GET"] || [method isEqual:@"HEAD"]) {
		...
		if ( ([[uri path] compare:@"/snapshot" options:NSCaseInsensitiveSearch] == NSOrderedSame) || ([[uri path] isEqualToString:@"/"])) {
			...
			CGImageRef image = [self allocMaskedScreenImage:screenImage];
			CGImageRelease(screenImage);
			
			if (([tokens count] >= 2) && ([[tokens objectAtIndex:0] compare:@"width" options:NSCaseInsensitiveSearch] == NSOrderedSame)) {
				width = [[tokens objectAtIndex:1] intValue];
				height = CGImageGetHeight(image) * width / CGImageGetWidth(image);
				CGImageRef scaledImage = [self newResizedCGImage:image toWidth:width andHeight:height];
			...
			CFHTTPMessageSetHeaderFieldValue(response, (CFStringRef)@"Content-Length", (CFStringRef)[NSString stringWithFormat:@"%ld", CFDataGetLength(data)]);
